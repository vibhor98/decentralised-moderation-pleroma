...
class SoftmaxLoss(nn.Module):
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor):
        reps = [self.model[1](self.model[0](sentence_feature))['sentence_embedding'] for sentence_feature in sentence_features]
        ...
        if labels is not None:
            loss = self.loss_fct(output[:, 1], labels.view(-1))
            return loss
        else:
            return reps, output
    ...
    def parent_child_agg(self, reps):
        u = reps[1]
        v = reps[0]
        return u, v
